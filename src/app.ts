...
export const handlePayload = async (request: Request, target: any) => {
  ...
  const args: any[] = [];
  //# Grab metadata keys
  const requestBodyParam = Reflect.getOwnMetadata(
    requestBodyKey,
    prototype,
    methodName,
  );
  const headersParam = Reflect.getOwnMetadata(
    headersKey,
    prototype,
    methodName,
  );
  ...
  const requestParameterParam: Record<
    string,
    { index: number; parameterType: string }
  > = Reflect.getOwnMetadata(requestParameterKey, prototype, methodName);
  const headerParam: Record<string, { index: number; parameterType: string }> =
    ...
    const result = await handlePayload(
      {
        ...
        requestParameters: request.queryStringParameters as Record<
          string,
          string
        >,
      },
