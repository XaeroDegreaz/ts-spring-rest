...
const createMapping = ( httpMethod: string, mapping: RequestMapping, prototype: any, methodName: string, descriptor: PropertyDescriptor ) => {
  let currentParams: RequestMappingTree = Reflect.getOwnMetadata( requestMappingKey, prototype, requestMappingTree ) || {[httpMethod]: {}}
  if ( !currentParams[httpMethod] )
  {
    currentParams[httpMethod] = {[mapping.path]: {method: descriptor.value, methodName}}
  }
  else
  {
    currentParams[httpMethod][mapping.path] = {method: descriptor.value, methodName}
